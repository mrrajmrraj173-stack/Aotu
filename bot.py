...
FEATURES
========
✅ Monitor multiple channels (Public + Private)
...
✅ HTTP Custom / V2RAY Profile support
✅ SSH Config parser
✅ Stable + async safe
"""
...
BOT_TOKEN = "8285296504:AAHL7LhsBgXMkHEN1PZCcXqYPRGBSiFrLH0"
...
def build_trojan(outbound):
    ...
    try:
        ...
        logging.error(
            f"TROJAN BUILD ERROR: {e}"
        )

        return None

# =========================================================
# JSON PARSER (WITH V2RAY PROFILE & SSH CONFIG SUPPORT)
# =========================================================
...
def parse_json(data, results):
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            # =================================================
            # HTTP CUSTOM / V2RAY PROFILE PARSER
            # =================================================
            ...
            if "sshConfig" in data:
                ...
                try:
                    ...
                    port = ssh.get("sshPort", 22)
                    userx = ssh.get("sshUsername", "")
                    password = ssh.get("sshPassword", "")

                    if host and userx and password:
...
# =========================================================
# ADD CHANNEL (UPDATED WITH PRIVATE INVITE SUPPORT)
# =========================================================
...
async def add_channel(event):
    ...
    for ch in args:
        ...
        # NORMALIZE LINKS
        if original.startswith("https://t.me/"):
            original = (
                original
                .replace("https://t.me/", "")
                .strip("/")
        ...
        elif original.startswith("http://t.me/"):
            original = (
                original
                .replace("http://t.me/", "")
                .strip("/")
        ...
        if (
            ...
            and not original.startswith("-100")
        ):
            original = f"@{original}"

        try:
            
            # JOIN FIRST TO GET ENTITY
            entity = None
            
            # PRIVATE INVITE
            if original.startswith("+"):
                
                invite_hash = original.replace("+", "")
                
                await user(ImportChatInviteRequest(invite_hash))
                
                # GET THE CHAT BY INVITE LINK
                entity = await user.get_entity(original)
                
            else:
                
                # PUBLIC OR PRIVATE WITH USERNAME
                await user(JoinChannelRequest(original))
                
                entity = await user.get_entity(original)
            
            # DETERMINE FINAL IDENTIFIER
            if getattr(entity, "username", None):
                final_id = f"@{entity.username}"
            else:
                final_id = str(entity.id)
            
            # ADD TO CONFIG
            if final_id not in config["source_channels"]:
                
                config["source_channels"].append(final_id)
                
                added.append(original)
                
                logging.info(f"✅ ADDED: {original} -> {final_id}")
                
        except Exception as e:
            
            logging.error(f"JOIN ERROR for {original}: {e}")
            
            # FALLBACK: just add original if it's already a valid format
            if original not in config["source_channels"]:
                config["source_channels"].append(original)
                added.append(original)
    ...
    if added:
        ...
        await event.reply("Already added or join failed")
...
# =========================================================
# MONITOR CHANNELS (UPDATED WITH PRIVATE CHANNEL SUPPORT)
# =========================================================
...
async def monitor(event):
    ...
    try:
        ...
        # NOT MONITORED
        if chat_identifier not in config["source_channels"]:
            return
        
        msg = event.message
        
        unique_id = f"{chat_identifier}_{msg.id}"
